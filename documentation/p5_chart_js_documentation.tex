...
The browser build installs a version-neutral add-on after p5 loads. Public methods are available in global mode and on each instance. Mutable tooltip, input, and geo state belongs to the current p5 instance.

\subsection{Global Properties}
Shared defaults are accessible through \texttt{p5.prototype.chart}. Instance overrides and mutable state are accessible through \texttt{p.chart}.
...
\texttt{chart.inputs} & Instance-owned cache for DOM elements (e.g., Table search inputs). \\
\texttt{chart.hoverState} & Instance-owned tooltip state (\texttt{active}, \texttt{x}, \texttt{y}, \texttt{content}, \texttt{tooltipScale}, \texttt{tooltipTextSize}, \texttt{tooltipFont}). \\
\bottomrule
\end{longtable}

\subsection{Loading Methods}
Loading follows the capabilities of the installed p5 version.

\begin{description}
    \item[\texttt{loadDataFrame(path, success, error)}] \hfill \\
    Loads a CSV file and converts it to a DataFrame. On p5.js 2.x it returns a Promise and is intended for \texttt{async setup()}. On p5.js 1.x it returns a DataFrame placeholder for existing \texttt{preload()} usage. Optional callbacks work in both versions.
\end{description}
...
\texttt{new p5.prototype.chart.DataFrame(data, [columns])} or \texttt{new p.DataFrame(data, [columns])}
\begin{itemize}
...
\begin{description}
    ...
    \item[\texttt{tableToDataFrame(pathOrTable, type, options)}] Converts a p5.Table synchronously in both versions. A path follows the version-specific loading behavior of \texttt{loadDataFrame()} and returns a Promise on p5.js 2.x.
\end{description}

\section{Chart Visualization Methods}

All chart methods follow the signature \texttt{method(data, options)} in global mode or \texttt{p.method(data, options)} in instance mode. The \texttt{data} argument accepts a DataFrame, an Array of objects, or a p5.Table. The \texttt{options} argument is a key--value object that describes how the chart should look and behave.
...
\textbf{NaN Behavior:} Displays \texttt{\textemdash} in gray.
